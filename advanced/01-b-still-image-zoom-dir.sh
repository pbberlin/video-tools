...
for img in ./${inpDir}/*.jpg; do
  ...
  # top
  outPath4="./${outDir}/${flnBase}-t.mp4"
  ffmpeg -loop 1 -i "$img" \
         -vf "zoompan=z='min(zoom+0.001,1.2)':x='iw/2-(iw/zoom/2)':y='ih/3.5-(ih/zoom/2)':d=30*4,scale=w=3840:h=2880,setsar=1/1" \
         -c:v libx264 -s 3840x2880 -t 4.5 -r 30 "$outPath4"

  # right
  outPath4="./${outDir}/${flnBase}-r.mp4"
  ffmpeg -loop 1 -i "$img" \
         -vf "zoompan=z='min(zoom+0.001,1.2)':x='iw/1.7-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=30*4,scale=w=3840:h=2880,setsar=1/1" \
         -c:v libx264 -s 3840x2880 -t 4.5 -r 30 "$outPath4"
